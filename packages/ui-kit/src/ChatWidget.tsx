...
function parseInline(text: string, baseKey: string): ReactNode {
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (match[2] !== undefined) {
      parts.push(<strong key={`${baseKey}-b${i++}`}>{match[2]}</strong>);
    } else if (match[3] !== undefined) {
      parts.push(<em key={`${baseKey}-i${i++}`}>{match[3]}</em>);
...
function parseMarkdown(text: string): ReactNode {
  ...
  const flushList = () => {
    if (listBuffer.length > 0) {
      ...
      listBuffer.length = 0;
      blocks.push(
        <ul key={`list-${blockKey++}`} className={styles.bubbleList}>
          {items.map((item, i) => (
            <li key={i} className={styles.bubbleListItem}>
              {parseInline(item, `li-${blockKey}-${i}`)}
            </li>
          ))}
        </ul>
      );
  ...
  for (const line of lines) {
    ...
    if (bulletMatch) {
      ...
      if (trimmed.length > 0) {
        blocks.push(
          <p key={`p-${blockKey++}`} className={styles.bubbleParagraph}>
